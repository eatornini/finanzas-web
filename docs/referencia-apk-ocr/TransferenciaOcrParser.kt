...
data class TransferenciaResult(
    val comercio: String?,
    val monto: Double?,
    val fecha: Long?,
    val detalle: String?
...
class TransferenciaOcrParser {
    ...
    fun detectAndParse(
        lines: List<OcrHelper.LineInfo>,
        @Suppress("UNUSED_PARAMETER") blocks: List<OcrHelper.BlockInfo>
    ): TransferenciaResult? {
        ...
        val detalle = extractDetalle(lines, allTextLower)
    ...
    private fun extractComercio(
        ...
        val destinoKeywords = listOf("para", "nombre", "destinatario",
    ...
    private fun findLabelValue(
        ...
        for (i in lines.indices) {
            ...
            for (label in labels) {
                ...
                val sep = "[:>\\->\u2192]"
                val colonPattern = Regex("""\b""" + labelLower + """\s*""" + sep + """\s*(.+)""", RegexOption.IGNORE_CASE)
    ...
    private fun esLabelLine(text: String, labels: List<String>): Boolean {
        ...
        return labels.any { lower == it.lowercase(Locale.ROOT)
    ...
    private fun extractFecha(
        lines: List<OcrHelper.LineInfo>
    ): Long? {
        var fechaStr: String? = null
        var horaStr: String? = null

        val fechaValue = findLabelValue(lines, listOf("fecha y hora", "fecha/hora", "fecha", "fecha y hora de operacion"))
        val horaValue = findLabelValue(lines, listOf("hora", "h0ra"))

        if (fechaValue != null) {
            val combined = fechaValue + " " + (horaValue ?: "")
            val combinedHasTime = horaValue != null || timeRegex.containsMatchIn(fechaValue) || timeRegexFlex.containsMatchIn(fechaValue)
            if (combinedHasTime) {
        ...
        val dateLine = lines.find { line ->
            dateRegex.containsMatchIn(line.text)
        }
        if (dateLine != null) {
        ...
        if (horaGlobal != null) {
            val globalDateLine = lines.find { line -> dateRegex.containsMatchIn(line.text) } ?: return null
    ...
    private fun buscarHoraTrasLabelFechaYHora(lines: List<OcrHelper.LineInfo>): String? {
    ...
    private fun buscarHoraEnLineasCercanas(lines: List<OcrHelper.LineInfo>): String? {
        for (i in lines.indices) {
    ...
    private fun buscarHoraGlobal(lines: List<OcrHelper.LineInfo>): String? {
        for (line in lines) {
    ...
    private fun parseFechaCompleta(text: String): Long? {
        try {
            ...
            val second = if (timeMatch?.groupValues?.size != null && timeMatch.groupValues.size >= 4) timeMatch.groupValues[3].toIntOrNull() ?: 0 else 0
    ...
    private fun extractDetalle(
        @Suppress("UNUSED_PARAMETER") lines: List<OcrHelper.LineInfo>,
        @Suppress("UNUSED_PARAMETER") allTextLower: String
    ): String? {
        return null
    }

    private fun detectarBanco(text: String): String? {
        val bancos = listOf(
            "banco estado" to "BancoEstado",
            "bancoestado" to "BancoEstado",
            "bci" to "BCI",
            ...
            "banco falabella" to "Banco Falabella",
            "banco internacional" to "Banco Internacional",
            "banco security" to "Banco Security",
            "banco bice" to "Banco BICE",
            "banco consorcio" to "Banco Consorcio",
            "banco ripley" to "Banco Ripley"
        )
        for ((key, name) in bancos) {
            if (text.contains(key)) return name
        }
        return null
    }

    private fun extractTipoCuenta(
        allTextLower: String
    ): String? {
        val tipos = listOf(
            "cuenta rut" to "CuentaRUT",
            "cuentarut" to "CuentaRUT",
            "cuenta pro" to "Cuenta Pro",
            "cuentapro" to "Cuenta Pro",
            "cta cte" to "Cta. Cte.",
            "cta. cte." to "Cta. Cte.",
            "cuenta corriente" to "Cta. Cte.",
            "cuenta vista" to "Cuenta Vista",
            "cuenta de ahorro" to "Cta. Ahorro",
            "cuentaRUT" to "CuentaRUT",
            "cuentaRut" to "CuentaRUT"
        )
        for ((key, name) in tipos) {
            if (allTextLower.contains(key)) return name
        }
        return null
    }

    private fun extractNumeroOperacion(
        lines: List<OcrHelper.LineInfo>
    ): String? {
        val opValue = findLabelValue(lines, listOf("n° de operacion", "n° de operación",
            "numero de operacion", "número de operación", "numero de operación",
            "folio", "numero de folio", "número de folio"))
        if (opValue != null) {
            val digits = opValue.replace(Regex("""[^\d]"""), "")
            if (digits.length >= 4) return digits
        }

        val opPattern = Regex("""(?:n[°º]\s*(?:de\s+)?operacion|n[°º]\s*(?:de\s+)?operación|numero\s+de\s+operacion|número\s+de\s+operación|folio|operacion|operación)[:\s]*(\d{4,})""", RegexOption.IGNORE_CASE)
        for (line in lines) {
            val match = opPattern.find(line.text)
            if (match != null) {
        ...
        for (line in lines) {
            val text = line.text.trim()
            val digits = text.replace(Regex("""[^\d]"""), "")
            if (digits.length >= 6 && digits.length <= 12 && text.length <= digits.length + 3) {
                return digits
            }
        }
    ...
    private fun extractMensaje(
        lines: List<OcrHelper.LineInfo>
    ): String? {
        val msgValue = findLabelValue(lines, listOf("mensaje", "comentario"))
        if (msgValue != null && msgValue.length > 2) {
            return msgValue
        }
        return null
    }
